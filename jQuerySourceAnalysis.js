...
在不查阅资料的前提下，唯一让我觉得作者这么做的原因是因为效率问题。
以下是我的理解：

调用实例arr的方法concat时，首先需要辨别当前实例arr的类型是Array，在内存空间中寻找
Array的concat内存入口，把当前对象arr的指针和其他参数压入栈，跳转到concat地址开始执行。
...
也即是让类数组也能用到数组的方法（这就是call跟apply带来的另一种用法），尤其在jQuery里
边引用一些DOM对象时，也能完美的用这个方法去解决，妙！
*/
var arr = [];
...
jQuery.fn = jQuery.prototype = {
	...
	//jQ对象里边选取的DOM节点数目，有了这个属性就已经像“半个”数组了，:)
	length: 0,
	//将jQ对象转换成数组类型，这里返回的结果就真的是Array类型了
	toArray: function() {
	...
	//获取索引为num的元素，看代码就知道这里支持num为负数
	//当num为负数的时候，相当于从数组尾巴倒数索引
	get: function( num ) {
		return num != null ?
	...
	pushStack: function( elems ) {
		...
		//这样ret就有了jQ对象的所有方法，也就是ret就是一个jQ对象了
		//并且ret里边的元素已经合并了elems了！
		var ret = jQuery.merge( this.constructor(), elems );

		// Add the old object onto the stack (as a reference)
		//为什么需要prevObject，这里看end方法就知道了，:)
		ret.prevObject = this;
	...
	//用eq的差别跟get的差别是：
	//get返回的是jQ对象里边的元素，元素类型是DOM节点
	//而eq方法拿到DOM节点后，还会通过pushStack方法封装成一个jQ对象，以便可以继续用jQ的方法操作
	eq: function( i ) {
		var len = this.length,
			j = +i + ( i < 0 ? len : 0 );
	...
	//本来find函数已经使得链的上下文切换到.clr这个jQ对象了，为了最后能回到#id这个jQ对象
	//可以使用end方法来返回
	//这里的秘籍就是每个对象里边的prevObject保存着链中的上一个jQ对象
	end: function() {
...
jQuery.extend = jQuery.fn.extend = function() {
	...
	if ( typeof target === "boolean" ) {
		...
		target = arguments[ i ] || {};
		i++;// 跳过boolean和target，从第3个开始，此时i=2
	}

	// Handle case when target is a string or something (possible in deep copy)
	// 如果target不是对象也不是函数，则强制设置为空对象
		...
		if ( (options = arguments[ i ]) != null ) {
			...
			for ( name in options ) {
				...
				if ( deep && copy && ( jQuery.isPlainObject(copy) || (copyIsArray = jQuery.isArray(copy)) ) ) {
					if ( copyIsArray ) {// 如果copy的是数组
						copyIsArray = false;
						clone = src && jQuery.isArray(src) ? src : [];// clone为src的修正值

					} else {//如果copy的是纯对象
						clone = src && jQuery.isPlainObject(src) ? src : {};// clone为src的修正值
					}
...
jQuery.extend({
	...
	expando: "jQuery" + ( version + Math.random() ).replace( /\D/g, "" ),//替换非数字字符为空
	...
	//根据 ECMAScript 标准要求，所有的 JavaScript 解释器都要将Window对象视作全局对象(global)，
	//并且global.window = global(其window属性引用自身)。
	//根据该标准，jQuery使用如下代码来判断指定对象是否是一个Window对象。
	...
	isPlainObject: function( obj ) {
		...
		//constructor是对创建对象的函数的引用（指针）。
		//对于 Object 对象，该指针指向原始的 Object() 函数
		//判断obj是否具有isPrototypeOf属性，isPrototypeOf只会存在于Object.prototype上。
		//也就是说如果obj的原型中存在isPrototypeOf则obj是Object的直接实例。
		//当然除非人为的将obj的构造函数的prototype.constructor置为Object,
		//或者将构造函数的prototype置为{}
		if ( obj.constructor &&//有constructor属性
				//并且原型中有isPrototypeOf方法，一般只有Object的原型中才有这个方法
	...
	isEmptyObject: function( obj ) {
		var name;
		for ( name in obj ) {
			//只要for循环执行了，说明obj不是空的否则返回true,这种判断很简单没有判断参数
			//是否是对象的情况下就直接循环了，你甚至可以用它来判断是否是空字符串
			return false;
		}
		return true;
	},
	...
	type: function( obj ) {
		...
		// toString.call(123) 实际上会返回一个字符串"[object Number]",此时就可以通过class2type表来返回"number"
	...
	each: function( obj, callback, args ) {
		...
		if ( args ) {//如果args存在，即为内部使用，这部份不分析
			if ( isArray ) {
				for ( ; i < length; i++ ) {
					//相当于:
					//args = [arg1, arg2, arg3];
	...
	trim: function( text ) {
		return text == null ?
			"" :
			( text + "" ).replace( rtrim, "" );// 删除 BOM and NBSP 
	},
	...
	// 对象的全局GUID计数器
	//唯一标识符，跟事件有关。举个例子：function show(){alert(this);}， $("#input1").click(show)，$("#input2").click(function(){$("#input1").off()})，
	//这里的show方法是事件方法，所以通过off取消掉事件绑定，可以很容易找到事件方法show。
	//但是如果把 $("#input1").click(show)改成 $("#input1").click($.proxy(show,window))，
	//这时show不是事件方法，而是普通方法，那么通过off取消的时候，它是怎么找到这个普通方法show的，
	//其实就是通过guid，因为guid会累加，所以是唯一的，因此可以找到。请看下个方法就知道详情了。
	guid: 1,
	...
	proxy: function( fn, context ) {
		...
		//这里处理特殊调用情况，比如：$.proxy(obj,"show")（正常写法$.proxy(obj.show,obj)），show方法执行时，this指向的obj，并且show是obj的属性方法.var obj = { show: function(){}}。
...
function isArraylike( obj ) {
	...
	if ( obj.nodeType === 1 && length ) {
		//元素节点对象，并且有length属性，返回真。document.getElementsByTagName("div")和body.childNodes都不是这种情况。
		//可能用于内部调用，这里如果有谁知道的，可以告诉我。
		return true;
	}
...
var rootjQuery,
	...
	init = jQuery.fn.init = function( selector, context ) {
		...
		if ( !selector ) {
			return this;//传过来如果为假,返回jQuery对象.注意这里返回的是jQuery域,而不是init的域!
		}
		...
		if ( typeof selector === "string" ) {
			...
			if ( selector[0] === "<" && selector[ selector.length - 1 ] === ">" && selector.length >= 3 ) {
				...
				//干吗的? 在上面定义的
				//quickExpr = /^(?:[^#<]*(<[\w\W]+>)[^>]*$|#([\w\-]*)$)/,
				//如果你熟悉正则表达式那你应该知道是干吗的。比如你执行 $("#test")，
				//那么返回数组["#test",null,"test"],当然这只是其中一个
		...
		// HANDLE: $(DOMElement)
		} else if ( selector.nodeType ) {//nodeType 如果为真 说明 selector已经是一个对象了
...
jQuery.Callbacks = function( options ) {
	...
	// options 这里设计为一个对象，这一步就是先判断下options缓存里有没这个对象存在，有直接拿来用，
	...
	var // Last fire value (for non-forgettable lists)
		...
		fire = function( data ) {
			...
			for ( ; list && firingIndex < firingLength; firingIndex++ ) {
				//遍历回调数组，执行每一个回调，当回调返回false且有传入stopOnFalse时，也就是$.Callbacks('stopOnFalse'),中止后面回调的执行
				if ( list[ firingIndex ].apply( data[ 0 ], data[ 1 ] ) === false && options.stopOnFalse ) {
					//当$.Callbacks('memory stopOnFalse')时，memory的作用将失效
					memory = false; // To prevent further calls using add // 阻止未来可能由于add所产生的回调
			...
			if ( list ) {
				if ( stack ) {//执行完回调后，看一下stack是否有回调，有拿出来执行
					if ( stack.length ) {
						fire( stack.shift() ); //从堆栈头部取出，递归fire
					}
				} else if ( memory ) {//否则，如果有记忆//如果没有stack，证明传了once，这里的Callbacks会是这样：$.Callbacks('once memory')
					list = [];
				} else {//再否则阻止回调列表中的回调 //当是$.Callbacks('once')的时候
					self.disable();
				}
			}
		},
		...
		self = {
			...
			add: function() {
				if ( list ) {
					...
					(function add( args ) {//这里用了一个立即执行的add函数来添加回调,直接遍历传过来的arguments进行push
						jQuery.each( args, function( _, arg ) {//jQuery.each,对args传进来的列表的每一个对象执行操作
							var type = jQuery.type( arg );
							if ( type === "function" ) {
								if ( !options.unique || !self.has( arg ) ) { //当$.Callbacks('unique')时，保证列表里面不会出现重复的回调
									list.push( arg );
								}
							//假如传过来的参数为数组或array-like，则继续调用添加，从这里可以看出add的传参可以有add(fn),add([fn1,fn2]),add(fn1,fn2)
			...
			fireWith: function( context, args ) {
				if ( list && ( !fired || stack ) ) {
					...
					if ( firing ) {//如果正在回调
						//firing表示执行中，如果是在执行中，则将其推入stack，stack在这里相当于一个缓存数组，用于当fire忙时暂存下回调，但我也是整不出一段代码让这里执行到，暂时不明白
...
jQuery.extend({
	...
	Deferred: function( func ) {
		var tuples = [
				...
				//分别对应：
				//事件  暴露的添加回调的接口名   Callbacks队列   处理结束后的状态
			...
			promise = {
				...
				//可以看到，无论是成功失败，参数里边的回调都会被加入成功|失败队列
				always: function() {
					...
					return jQuery.Deferred(function( newDefer ) {
						jQuery.each( tuples, function( i, tuple ) {
							...
							deferred[ tuple[1] ](function() {
								var returned = fn && fn.apply( this, arguments );//当前的this == deferred
				...
				//还记得例子中的 return deferred.promise()
				//就是因为不想把[resolve | reject | notify]这几个函数暴露出去
				promise: function( obj ) {
					return obj != null ? jQuery.extend( obj, promise ) : promise;
				}
			},
			deferred = {};//最终生成的异步队列实例
		...
		jQuery.each( tuples, function( i, tuple ) {
			...
			//可以看到 done|fail|progress其实就是Callbacks里边的add方法
		...
		var i = 0,
			...
			updateFunc = function( i, contexts, values ) {
				return function( value ) {
					...
					} else if ( !( --remaining ) ) {//成功，并且最后剩余的异步任务为0了
						//说明所有任务都成功了，派发成功事件出去
...
jQuery.fn.ready = function( fn ) {
	// Add the callback
	//这里的jQuery.ready.promise()返回的就是异步队列
    //调用异步队列的done方法，把fn回调加入成功队列里边去
...
jQuery.extend({
	...
	//可以使用holdReady接口来hold住ready事件
	readyWait: 1,

	// Hold (or release) the ready event
	//可以使用holdReady接口来hold住ready事件
    //参数hold为true时，jQuery会hold住ready回调。
    //某个地方调用了holdReady()，参数hold为false，jQuery会把hold住的次数减1
	...
	ready: function( wait ) {
		//如果不需要等待，holdReady()的时候，把hold住的次数减1，如果还没到达0，说明还需要继续hold住，return掉
		...
		//当时如果此时有某个地方hold住了，那就要继续等待
		if ( wait !== true && --jQuery.readyWait > 0 ) {
			return;
		}

		// If there are functions bound, to execute
		//调用异步队列，然后派发成功事件出去（最后使用done接收，详细见《jQuery源码剖析（三）——$.Deferred》），把上下文切换成document，默认第一个参数是jQuery。
		//还记得 $(document).ready(function($){})这里的$不？
...
jQuery.ready.promise = function( obj ) {//这里的obj是可以扩展返回的异步队列的，
	//readyList就是内部用于ready的一个异步队列
	if ( !readyList ) {
		//如果没有，就用jQuery.Deferred()生成一个异步队列
		readyList = jQuery.Deferred();

		// Catch cases where $(document).ready() is called after the browser event has already occurred.
		// We once tried to use readyState "interactive" here, but it caused issues like the one
		// discovered by ChrisS here: http://bugs.jquery.com/ticket/12282#comment:15
		// Chrome下document.readyState == interactive
		// IE下document.readyState == complete
		//这里是针对IE做的
	...
	//调用异步队列的promise方法隐藏一些触发事件接口
	//因为触发ready事件的控制权应该是jQuery自己管理
...
var access = jQuery.access = function( elems, fn, key, value, chainable, emptyGet, raw ) {
	var i = 0,
		len = elems.length,
		bulk = key == null;//此处的 bulk 指的是批量操作，key 为 null，就是说当前的操作即……

	// Sets many values
	//key 为对象，就表明这是一个类似于 { height: 100, width: 200 } 的键值对，是将多步操作合并在了一起，
	//而且这样的操作不能是 get 类方法，因为你没法确定最终的返回值是什么，所以 chainable 设置为 true，并且循环 key 对象来分别调用 access 方法。
...
jQuery.fn.extend({
	...
	queue: function( type, data ) {
		var setter = 2;// 表示参数数量，默认设为2

		// 如果type不是字符串，则默认当做是data来用，而type变为默认值“fx”
		if ( typeof type !== "string" ) {
			data = type;
			type = "fx";
			setter--;// 原data废弃，因此setter减小1
		}
		...
		return data === undefined ?
			this :
			this.each(function() {// data为有效值，则调用each方法
	...
	promise: function( type, obj ) {
		var tmp,
			...
			elements = this,// 引用JQ对象
			i = this.length,// 获取JQ对象的长度
...
/*
事件处理核心是运用了data缓存，增加事件监听时，在每一个JQ对象的data缓存中都
保存一个对象map（事件类型-回调数组），事件触发后，会遍历执行回调数组里的每
一个回调，移除监听，则是删除对应缓存里的回调或事件类型，以上三种场景构成整一个jQuery事件处理流程。
*/
/*
...
jQuery.event = {
	...
	add: function( elem, types, handler, data, selector ) {
		...
		while ( t-- ) {
			...
			// 取出事件处理类型，如click
			type = origType = tmp[1];
			// tmp[2]是命名空间，如bbb.ccc，并根据"."分隔成数组
            ...
            // extend内部确定需要扩展的target时，使用的是形参长度，因此只要用来扩展的对象传了形参，即使为空，
			// 也不会变为将handleObj扩展进jQuery只当传了一个参数时，target才会变为this，即jQuery本身。
			...
			if ( !(handlers = events[ type ]) ) {
				...
				//如果存在则将会把控制权限交给自定义事件的事件初始化函数，同样事件卸载的jQuery.event.remove方法在删除处理器前也会检查此。
			...
			// 将事件处理函数推入处理列表 
			if ( selector ) {
				// 这里使用splice时插入对象（handlers.delegateCount是位置，第二个参数是0表明不删除
				// 第三个是需要插入数组的对象，会被放在handlers.delegateCount前面）
